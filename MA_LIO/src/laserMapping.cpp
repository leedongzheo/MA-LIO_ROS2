...
void standard_pcl_cbk(const std::shared_ptr<const sensor_msgs::msg::PointCloud2> &msg, int num_lidar)
{
    mtx_buffer.lock();
...
void livox_pcl_cbk(const std::shared_ptr<const livox_ros_driver2::msg::CustomMsg> &msg, int num_lidar)
{
    mtx_buffer.lock();
    last_timestamp_lidar = rclcpp::Time(msg->header.stamp).seconds();
    PointCloudXYZI::Ptr ptr(new PointCloudXYZI());
...
void lidar_cbk_(const std::shared_ptr<const sensor_msgs::msg::PointCloud2> &scanMsg_,
                  const std::shared_ptr<const livox_ros_driver2::msg::CustomMsg> &livoxMsg_, 
                  const std::shared_ptr<const livox_ros_driver2::msg::CustomMsg> &livoxMsg2_)
  {
...
int main(int argc, char **argv)
{
    ...
    // ĐOẠN CODE KIỂM TRA TẠM THỜI
    std::cout << "--- KIỂM TRA KÍCH THƯỚC MẢNG THAM SỐ ---" << std::endl;
    std::cout << "Kích thước extrinT: " << extrinT.size() << std::endl;
    std::cout << "Kích thước extrinR: " << extrinR.size() << std::endl;
    std::cout << "Kích thước lid_type: " << lid_type.size() << std::endl;
    std::cout << "Kích thước N_SCANS: " << N_SCANS.size() << std::endl;
    std::cout << "Kích thước point_filter_num: " << point_filter_num.size() << std::endl;
    std::cout << "----------------------------------------" << std::endl;
    ...
    while (status)
    {
        ...
        {   /*** Initialize ***/
            ...
            // TẠO BIẾN LƯU THỜI GIAN AN TOÀN
            // double current_log_time = 0.0;
            // if (Measures.lidar_end_time.size() > 2) {
            //     current_log_time = Measures.lidar_end_time[2]; // Nếu đủ 3 LiDAR thì lấy thời gian của LiDAR 3
            // } else if (Measures.lidar_end_time.size() > 0) {
            //     current_log_time = Measures.lidar_end_time[0]; // Nếu thiếu thì lấy tạm thời gian của LiDAR 1
            // }
            // fout_out << current_log_time << " " << state_point.pos(0) << " " << state_point.pos(1) << " " << state_point.pos(2) << " " << geoQuat.x << " " << geoQuat.y << " " << geoQuat.z << " " << geoQuat.w << std::endl;
