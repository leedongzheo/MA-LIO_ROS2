...
#include "ROSThread.h"
// BỔ SUNG 3 HEADER NÀY ĐỂ KÍCH HOẠT MAP TRƯỜNG 'RING' SANG ROS2 JAZZY
...
void ROSThread::run()
{
  // CHỐT CHẶN AN TOÀN: Nếu node_ chưa khởi tạo, bắt luồng đợi một chút, tránh sập nguồn vùng nhớ
  while (rclcpp::ok() && !node_) {
...
//    data_stamp_[stamp] = data_name;
  ...
  // BỔ SUNG 2 DÒNG NÀY ĐỂ DẬP TẮT LỖI MẢNG RỖNG (SIZE = 0)
...
void ROSThread::DataStampThread()
{
  ...
  for(auto iter = data_stamp_.begin() ; iter != data_stamp_.end() ; iter ++){
    ...
    if(prev_clock_stamp_ == 0 || (stamp - prev_clock_stamp_) > 10000000){
        ...
        // Gán giây và nano giây từ stamp vào cấu trúc builtin_interfaces::msg::Time của ROS2
        clock_msg.clock = rclcpp::Time(stamp);
